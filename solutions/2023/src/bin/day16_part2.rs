...
fn solve(input: &str) -> Result<usize> {
    ...
    for col in 0..width {
        starts.push((Position { row: 0, col }, South));
        starts.push((
            Position {
                row: height - 1,
                col,
    ...
    for row in 0..height {
        starts.push((Position { row, col: 0 }, East));
        starts.push((
            Position {
                row,
                col: width - 1,
    ...
    for start in starts {
        let energy = calculate_energized_tiles(&map, start);
        answer = answer.max(energy);
    }

    // let energy = calculate_energized_tiles(&map, start)

    // calculate_energized_tiles(&map);

    Ok(answer)
}

fn calculate_energized_tiles(map: &[Vec<Tile>], start: (Position, Direction)) -> usize {
    let width = map[0].len();
    let height = map.len();

    let mut map = map.to_vec();
    energize_tiles(&mut map, &start);
...
fn energize_tiles(map: &mut [Vec<Tile>], start: &(Position, Direction)) {
    let width = map[0].len();
    let height = map.len();

    let mut queue = VecDeque::new();
    map[start.0.row][start.0.col]
        .light_rays
        .push(LightRay { direction: start.1 });
    map[start.0.row][start.0.col]
        .unfollowed_rays
        .push(LightRay { direction: start.1 });
